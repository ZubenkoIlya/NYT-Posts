//
//  MostSharedPostsViewController.swift
//  NYT Posts
//
...
class MostSharedPostsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, ArticleViewDelegate {
    ...
    init() {
        super.init(nibName: "MostSharedPostsViewController", bundle: nil)
    }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return sharedPostsArray.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "PostTableViewCell", for: indexPath) as! PostTableViewCell
        
        let post = sharedPostsArray[indexPath.row]
        ...
        cell.articleImageViewCell.af_setImage(
            ...
            completion: { response in
                sharedPostsArray[indexPath.row].mainImage = response.value!
        })
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        openArticleView(shared: sharedPostsArray[indexPath.row])
    }
    ...
    func openArticleView(shared: SharedNewsPost?) {
        if shared != nil {
            self.articleView.articleImageView.image = shared?.mainImage
            self.articleView.articleTitleLabel.text = shared?.title
            self.articleView.articleAbstractLabel.text = shared?.abstract
            self.articleView.sharedItem = shared
            self.articleView.toFavoriteButton.isEnabled = true
            for favoritePost in favoritePostsArray {
                if favoritePost.webLink == shared?.url {
    ...
    func toFavorites(emailedPost: EmailedNewsPost?, sharedPost: SharedNewsPost?, viewedPost: ViewedNewsPost?) {
        NewsPostCoreDataManager.shared.saveImageOnDevice(image: sharedPost!.mainImage) { (imagePath) in
            NewsPostCoreDataManager.shared.savePostToCoreData(title: sharedPost!.title, abstract: sharedPost!.abstract, imagePath: imagePath, weblink: sharedPost!.url, closure: {
